...
contract MemoryLifecycle {
    ...
    */
    function addStorageNumber(
        uint256 _number
    )
        public
    {
        storageNumbers.push(_number);
    }
...
Reason:
Developers may assume memory updates
...
The contract copies storage data into
memory.

Changes made to memory variables do not
affect storage.

Developers unfamiliar with Solidity may
incorrectly expect memory modifications
to persist.
...
3. Modify memory variable

4. Read storedName again

Result:
storedName remains unchanged

Root Cause:

- Memory creates copy
- Storage and memory behave differently

Recommendation:

Use storage when persistent updates
are required.

Use memory for temporary data only.
