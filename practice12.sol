...
Title:
Missing Access Control On Persistent State

Severity:
Medium

Reason:
Any user can permanently modify contract state.
...
The contract allows any user to modify the
counter variable.

Because storage persists permanently,
malicious users can overwrite important values.
...
Restrict state updates to owner
and track previous state before updates.
